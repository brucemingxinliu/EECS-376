...
void laserCallback(const sensor_msgs::LaserScan& laser_scan) {
    if (ping_index_<0)  {
        ...
        ping_index_start = (-1.57 -angle_min_)/angle_increment_;
        ping_index_end = (1.57 -angle_min_)/angle_increment_;
        ROS_INFO("LIDAR setup: i = %d , r = %d",ping_index_start, ping_index_end);
        
    }
    
   for (double i = ping_index_start; i <= ping_index_end; angle_increment_){
