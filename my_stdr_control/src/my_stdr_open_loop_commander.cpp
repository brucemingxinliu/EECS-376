...
int main(int argc, char **argv) {
    ...
    double yaw_rate = 0.26; //0.5 rad/sec yaw rate command
    double bigger_yaw_rate = 0.767;
    double time_3_sec = 3.0; // should move 3 meters or 1.5 rad in 3 seconds
    double counter_yaw_rate = -0.18;
...
//added
    ...
    twist_cmd.angular.z=bigger_yaw_rate; //and start spinning in place
    timer=0.0; //reset the timer
    while(timer<time_3_sec) {
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
    ...
    twist_cmd.linear.x=0.0; //stop moving forward
    twist_cmd.angular.z=0.0; 
    twist_cmd.angular.z=counter_yaw_rate; //and start spinning in place
    timer=0.0; //reset the timer
    while(timer<time_3_sec) {
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
    ...
    twist_cmd.angular.z=-0.295; //and start spinning in place
    timer=0.0; //reset the timer
    while(timer<3.0) {
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
